...
# not sure why the naming convention for ISO functions use julian rather than jd
...
def mayan_count_to_jd(baktun, katun, tun, uinal, kin):
    ...
    return (
        MAYAN_COUNT_EPOCH + (baktun * 144000) + (katun * 7200) + (tun * 360) + (uinal * 20) + kin
    )
...
def verify(jd, func, args_tuple):
    ...
    if jd != jd_cmp:
        print(
            "ERROR: %s(%s) = %s did not match jd (%s)" % (func, str(args).str(jd_cmp), str(jd))
        )
        return 0
    else:
        return 1
